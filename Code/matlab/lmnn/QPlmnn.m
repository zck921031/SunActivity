function [L,Det]=QPlmnn(x,y,varargin)
...
L=L(1:pars.outdim * length( pars.Lpart ), :);
...
switch(pars.obj)
  case 0    % updating Q
     ...
     L = zeros( size(L) );
     for modal = 1 : length(pars.Lpart)
        Qt = Q( pars.Lpart(modal, 1):pars.Lpart(modal,2), pars.Lpart(modal, 1):pars.Lpart(modal,2) );         
        [Lt, dd] = eig(Qt);
        dd=real(diag(dd));
        Lt = real(Lt);    
        j=find(dd<1e-10);
        ...
        L( (modal-1)*pars.outdim+1 : modal*pars.outdim, pars.Lpart(modal, 1):pars.Lpart(modal,2) ) = Lt';
     end
     % decompose Q
%      [L,dd]=eig(Q);
%      dd=real(diag(dd));

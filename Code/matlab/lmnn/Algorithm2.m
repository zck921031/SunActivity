...
if(~pars.quiet),fprintf('multi modal LMNN version 0.1\n');end;
...
switch(pars.obj)
  case 0    % updating Q
     ...
     for cc = 1 : size(pars.Lpart, 1)
        Qt = Q( pars.Lpart(cc, 3):pars.Lpart(cc, 4), pars.Lpart(cc, 3):pars.Lpart(cc, 4) );         
        [Lt, dd] = eig(Qt);
        dd=real(diag(dd));
        Lt = real(Lt);    
        j=find(dd<1e-10);
...
%           for j = pars.Lpart(cc,3):pars.Lpart(cc,4)
%               mmL(i,j) = L(i,j);
%           end
%       end
 end

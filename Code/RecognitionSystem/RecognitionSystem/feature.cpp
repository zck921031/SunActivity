...
vector<double> ColorHist(const Mat &img, int N){
...
void load_feature( string path, vector< vector<double> >&x, vector<int>&y, double ratio ){
...
void load_feature( string path, vector< vector<double> >&x, vector<int>&y, vector<Sift_BOW>&sift_bows, double ratio )
{
	...
	int cnt = 0;
	for ( string waveName : modal ){
		auto g = csvread( path + "//" + waveName + ".center.txt");

...
void load_feature( string path, vector< vector<double> >&x, vector<int>&y, vector< vector<double> >&xTe, vector<int>&yTe, double ratio ){
...
void load_feature_ox5( string path, vector< vector<double> >&x, vector<int>&y, vector< vector<double> >&xTe, vector<int>&yTe, double ratio ){
	vector< vector<double> > xtmp;
	vector<int> ytmp;
	map<int,int> ycnt;
	ifstream is(path + "//concept.txt");
...
int main(){
    string realpath;
    cout<<"Input feature folder path: ";
    char str[10240];
    cin.getline(str, 10240);
    realpath = str;
    matrix2d xTr, xTe;
    vector<int> yTr, yTe;
    load_feature_ox5( realpath, xTr, yTr, xTe, yTe, 0.5 );
    if ( xTr.empty() ){
        cout<<"no train data existed"<<endl;
        return 0;
    }

    int D = xTr[0].size();
    matrix2d A(D, vector<double>(D,0) );
    for(int i=0; i<D; i++){
        A[i][i] = 1;
    }
    double test_rate;
	test_rate = nca_debug::test(xTr, yTr, xTe, yTe, A);

	cout<<"before dml(NCA), accuracy is: "<<test_rate<<endl;

    long long tm = clock();
	A = nca_solve( xTr, yTr, D, 10, 1 );
	cout<<"used time = "<<clock()-tm<<endl;

	test_rate = nca_debug::test(xTr, yTr, xTe, yTe, A);
	cout<<"after dml(NCA), accuracy is: "<<test_rate<<endl;

    freopen("result.mat","w",stdout);
    for (auto ct : A){
        for ( auto t : ct ){
            cout<<t<<" ";
        }
